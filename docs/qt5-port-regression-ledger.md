...
| Area | Change | Risk | Suggested verification | Status |
| --- | --- | --- | --- | --- |
| Icon pixmap cache | Qt6 `QIcon::pixmap(size, dpr, mode, state)` replaced with Qt5 `pixmap(size * dpr, mode, state)` plus `setDevicePixelRatio(dpr)`. | Icons could render blurry, oversized, undersized, or cache incorrectly across scale factors and states. | Compare normal, disabled, checked, selected, and hovered icons at 100%, 150%, and 200% scaling. | Observed: user reports visuals are basically consistent. |
